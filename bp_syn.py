...
def norm1(vect):
    return [10*a/max(vect) for a in vect]
...
path = path_origin + '/Kumamoto/' + dossier + '/ligne'
...
path_results = path_origin + '/Kumamoto/' + dossier + '_results'
...
lst_fch = os.listdir(path)

os.chdir(path_origin + '/Kumamoto/syn')
with open('ligne_veldata', 'rb') as my_fch:
    my_dpck = pickle.Unpickler(my_fch)
    dict_vel = my_dpck.load()

dict_vel_used = dict_vel[0]
dict_delai = dict_vel[1]
...
with open('stack_vel_' + dt_type + '_env', 'wb') as my_fch_stk:
    my_pck_stk = pickle.Pickler(my_fch_stk)
    my_pck_stk.dump(stack)
