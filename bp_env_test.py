...
for station in list_file_used:
    st = read(station)
    if st[0].stats.station == cl_or_eq:
        t_or_eq = 5 - dist(dict_seis[dossier_seisme]['lat'], dict_seis[dossier_seisme]['lon'], - dict_seis[dossier_seisme]['dep'], st[0].stats.sac.stla, st[0].stats.sac.stlo, st[0].stats.sac.stel/1000)/v_P
        print(t_or_eq)
    env_norm = np.zeros(st[0].stats.npts)
   # env_norm[int(st[0].stats.sac.t0*st[0].stats.sampling_rate)] = 1
    env_norm[int((st[0].stats.sac.t0 + travt[ista][2, 10] - travt[ista][22, 10] + 40/3)*st[0].stats.sampling_rate)] = 1
 #   print(int((st[0].stats.sac.t0 + travt[ista][22, 10] - travt[ista][22, 10] + 0/3)*st[0].stats.sampling_rate))
    #env_norm[int((st[0].stats.sac.t0 + travt[ista][27, 10] - travt[ista][22, 10] + 10/3)*st[0].stats.sampling_rate)] = 1
    t_gauss = np.arange(st[0].stats.sampling_rate*2)/st[0].stats.sampling_rate
    t_gauss = [a - t_gauss.max()/2 for a in t_gauss]
 #   for hoho in range(len(env_norm)):
 #       if env_norm[hoho] > 0 or env_norm[hoho] < 0:
 #           print(hoho, env_norm[hoho])
 #   print(t_gauss)
 #   print(gauss(t_gauss, 0.5, 0))
    env_norm = np.convolve(env_norm, gauss(t_gauss, 0.5, 0), 'same')
 #   for gigi in range(len(env_norm)):
 #       if env_norm[gigi] > 0 or env_norm[gigi] < 0:
 #           print(gigi, env_norm[gigi])
    t = np.arange(len(env_norm))/st[0].stats.sampling_rate
    f = interpolate.interp1d(t, norm1(env_norm))
#    st = read(station)
...
os.chdir(path)
with open('test5_stack_acc_' + select_station, 'wb') as mon_fich_stack:
...
ax_3d.contour(Xy, np.transpose(stack[:, ymax, :]), Ty, 10, zdir = 'y', offset = w_fault/pas_w, cmap = 'jet')
...
ax_3d.set_xlabel('Strike (km)')
ax_3d.set_ylabel('Dip (km)')
ax_3d.set_zlabel('t (s)')
ax_3d.set_zlim3d(0, 10)
fig_3d.savefig('test5_plot_proj_3d.pdf')
...
ax_3d_2.set_xlabel('Strike (km)')
ax_3d_2.set_ylabel('Dip (km)')
ax_3d_2.set_zlabel('t (s)')
ax_3d_2.set_zlim3d(0, 10)
fig_3d_2.savefig('test5_plot_proj_3d_intersect.pdf')
...
for ij in range(int(length_t/5)):
    ...
    im_hyp = ax_bp.scatter(- dist_hyp*math.sin(d2r(strike - az_hyp))/pas_w/math.cos(d2r(dip)), (l_fault + dist_hyp*math.cos(d2r(strike - az_hyp)))/pas_l, s = 25, marker = '*', c = 'white')
    ...
    fig_bp.savefig('test5_' + dossier_seisme + '_acc_bp_' + str(m) + '_' + str(f_ech) + 'Hz.png')
...
fig_bptr.savefig('test5_bp_traces.pdf')
